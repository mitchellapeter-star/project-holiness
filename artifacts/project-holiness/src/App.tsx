...
// A small stylized Sacred Heart glyph: flame, radiating light, and a pierced heart. Uses
// currentColor so it can be tinted to fit wherever it's placed.
function SacredHeart({ size = 24, className = "" }: { size?: number; className?: string }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" className={className} aria-hidden="true">
    <path d="M9.2 2.7 8 1.4M14.8 2.7 16 1.4M12 0v1.3" stroke="currentColor" strokeWidth="0.7" strokeLinecap="round" />
    <path d="M12 1.7c.9 1.3 1.7 2.5 1.7 3.5 0 .95-.75 1.6-1.7 1.6s-1.7-.65-1.7-1.6c0-1 .8-2.2 1.7-3.5z" fill="currentColor" />
    <path d="M12 21.6c-4.9-3.4-9.2-7.4-9.2-11.9C2.8 6.4 5.2 4 8.2 4c1.7 0 3.3.9 3.8 2.4.5-1.5 2.1-2.4 3.8-2.4 3 0 5.4 2.4 5.4 5.7 0 4.5-4.3 8.5-9.2 11.9z" fillOpacity="0.14" fill="currentColor" stroke="currentColor" strokeWidth="1.1" strokeLinejoin="round" />
    <path d="M12 8.7v8.3M8.7 12.1h6.3" stroke="currentColor" strokeWidth="0.9" strokeLinecap="round" />
  </svg>;
}

// A quatrefoil rosette, echoing Gothic stained-glass window tracery. Used as a corner watermark.
function Quatrefoil({ size = 64, className = "" }: { size?: number; className?: string }) {
  return <svg width={size} height={size} viewBox="0 0 64 64" fill="none" className={className} aria-hidden="true">
    <g stroke="currentColor" strokeWidth="1.1">
      ...
      <circle cx="15" cy="32" r="13" />
      <circle cx="49" cy="32" r="13" />
      <circle cx="32" cy="32" r="7.5" />
    </g>
  </svg>;
}

// A horizontal rosary strand: a strung line of beads (with a larger bead every few) ending in
// a small cross. Used as an ornamental divider.
function RosaryStrand({ className = "", beadCount = 17 }: { className?: string; beadCount?: number }) {
  const beads = Array.from({ length: beadCount }, (_, i) => {
    const x = 6 + i * ((388 - 6) / (beadCount - 1));
    const y = 14 + Math.sin((i / (beadCount - 1)) * Math.PI) * 5;
    const big = i % 4 === 0;
    return { x, y, r: big ? 3.6 : 2.1 };
  });
  const path = `M ${beads.map(b => `${b.x.toFixed(1)} ${b.y.toFixed(1)}`).join(" L ")}`;
  return <svg viewBox="0 0 400 28" className={className} preserveAspectRatio="none" aria-hidden="true">
    <path d={path} stroke="currentColor" strokeWidth="0.8" fill="none" opacity="0.55" />
    {beads.map((b, i) => <circle key={i} cx={b.x} cy={b.y} r={b.r} fill="currentColor" opacity={b.r > 3 ? 0.9 : 0.6} />)}
    <g transform={`translate(396,14)`} stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" opacity="0.85">
...
function PageHeader({ eyebrow, title, description, action }: { eyebrow: string; title: string; description: string; action?: ReactNode }) {
  return <div className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end relative overflow-hidden rounded-3xl p-7 md:p-10 border border-[#DDD2C0] shadow-xl bg-[#F5F1E9]">
    <div className="absolute inset-0 opacity-[.55] mix-blend-multiply" style={{ backgroundImage: `url(${stainedGlassImg})`, backgroundSize: 'cover', backgroundPosition: 'center 30%' }} />
    <div className="absolute inset-0 bg-gradient-to-t from-[#F5F1E9] via-[#F5F1E9]/78 to-transparent pointer-events-none" />
    <div className="absolute top-0 right-0 -mr-4 -mt-4 opacity-[.07] pointer-events-none hidden md:block">
      <Cross size={180} strokeWidth={0.5} className="text-[#2D4C3C]" />
    </div>
    <div className="absolute bottom-0 left-0 -ml-6 -mb-6 opacity-[.06] pointer-events-none hidden md:block">
      <Quatrefoil size={140} className="text-[#8C6D23]" />
    </div>
    <div className="relative z-10"><p className="mb-2 flex items-center gap-1.5 font-mono text-[10px] font-semibold uppercase tracking-[.22em] text-[#8C6D23] drop-shadow-sm"><SacredHeart size={12} className="text-[#8C6D23]" />{eyebrow}</p><h1 className="font-serif text-3xl tracking-[-.035em] text-[#31231E] md:text-[40px] font-bold drop-shadow-sm">{title}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[#5C4D43]">{description}</p></div>{action && <div className="relative z-10">{action}</div>}
  </div>;
}
...
function Shell({ children, onSignOut, userEmail, displayName, avatarKey, onSaveProfile }: { children: ReactNode; onSignOut: () => void; userEmail: string | null; displayName: string | null; avatarKey: string | null; onSaveProfile: (name: string, avatar: string) => Promise<boolean> }) {
  ...
  return <div className="paper-grain min-h-[100dvh] bg-[#F5F1E9] text-[#31231E] holy-pattern rosary-pattern relative">
