...
const nav = [
  ...
  { href: "/a3", label: "Project Plan A3", icon: ClipboardCheck },
  { href: "/leader-standard-work", label: "Standard Work", icon: CalendarDays },
...
function GuidePage() {
  const a3Flow = [
    ...
    { label: "Standard Work", detail: "The rhythm you track over time" },
  ];
  return (
    <>
      <PageHeader
      ...
      <div className="w-full space-y-12">
        <section className="rounded-3xl border border-[#CDBD9D] bg-gradient-to-br from-[#EBE3D0] via-[#F5F1E9] to-white/70 p-6 shadow-md md:p-10">
          <div className="mb-7 max-w-3xl">
            ...
            <h2 className="font-serif text-2xl font-bold text-[#31231E]">What is an A3?</h2>
            <p className="mt-3 leading-relaxed text-[#5C4D43]">
              An A3 is a one-page thinking process. It moves from purpose, through honest diagnosis, into concrete action and a repeatable rule of life.
            </p>
          </div>
          <div className="flex flex-col items-stretch gap-2 lg:flex-row lg:items-center lg:gap-1.5" aria-label="A3 process flow">
        ...
        <section className="rounded-3xl border border-[#DDD2C0] bg-white/40 p-8 md:p-10 shadow-sm backdrop-blur">
          <h2 className="font-serif text-2xl font-bold text-[#31231E] mb-4">4. Action Items & Standard Work</h2>
          <p className="text-[#5C4D43] leading-relaxed mb-6">
            Countermeasures are theoretical until they become Action Items. Action Items are specific, scheduled tasks or recurring habits. Recurring action items automatically populate your <strong>Leader Standard Work</strong> grid, allowing you to track your daily, weekly, and monthly faithfulness.
          </p>
...
function A3Page({ store, setStore }: { store: Store; setStore: Dispatch<SetStateAction<Store>> }) {
  ...
  return <>
    <PageHeader eyebrow="Plan" title="Project Plan A3" description="The overarching plan for your spiritual life. Grounded in the universal call, tailored to your particular struggles and resolutions." />
      ...
      <section className="rounded-3xl border border-[#DDD2C0] bg-white/40 p-7 md:p-9 shadow-sm backdrop-blur">
        <div className="mb-6 flex items-center justify-between border-b border-[#DDD2C0] pb-4">
          <div>
            ...
            <p className="mt-1 text-sm text-[#827264]">Specific actions to implement the countermeasures. Active recurring items form your Standard Work.</p>
          </div>
...
function StandardWorkPage({ store, setStore }: { store: Store; setStore: Dispatch<SetStateAction<Store>> }) {
  ...
  return <>
    <PageHeader eyebrow="Leader Standard Work" title="The daily field." description="A generated view of your commitments based on your Project Plan A3." action={<div className="flex items-center justify-between gap-4 rounded-xl border border-[#DDD2C0] bg-white/70 p-1.5 shadow-sm"><Button variant="ghost" onClick={() => setMonth(shiftMonth(month, -1))}><ChevronLeft size={16} /></Button><span className="min-w-[120px] text-center font-mono text-[11px] font-bold uppercase tracking-widest text-[#5C4D43]">{monthLabel(month)}</span><Button variant="ghost" onClick={() => setMonth(shiftMonth(month, 1))}><ChevronRight size={16} /></Button></div>} />
